...
def main():
    ...
    for _, company in comparison.iterrows():
        ...
        company_result_start = len(results)
        ...
        company_id,
        "PRO_DEFAULT",
        "Company has available financial data for analysis.",
        60,
    )
         add_con(
        company_id,
        "CON_DEFAULT",
        "Limited financial-ratio data is available for the latest reporting period.",
        60,
    )
         continue
...
# ----------------------------------------------------
    ...
    company_ids = comparison["company_id"].unique()
    ...
    for company_id in company_ids:
        ...
        if not has_pro:
         add_pro(
            company_id,
            "PRO_DEFAULT",
            "Company has measurable financial metrics available for analysis.",
            60,
        )
        ...
        if not has_con:
         add_con(
            company_id,
            "CON_DEFAULT",
            "No specific negative financial signal was identified by the available screening rules.",
            60,
        )
